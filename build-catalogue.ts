...
const PAGE_IDS: Record<string, number> = {
  ...
  "orientation-usager": 433,
  "orientation-usager-v2": 635,
  "experiences-declarees-employeur": 85,
  "cadre-vie-communes": 91,
  "competences-professionnelles": 98,
  "open-formation": 106,
  "pe-connect-indemnisations": 117,
  "informations-territoire": 232,
  "rome-4-0-metiers": 273,
  "la-bonne-boite": 360,
  "contrat-engagement": 431,
  "declaration-demarche": 455,
  "gestion-activites-operationnelles": 617,
};
...
const CROSS_CONCEPTS = [
  { name: "Niveau d'étude", apis: ["orientation-usager","formations-professionnelles","offres-emploi"], collision: false, shared: true, description: "10 codes strictement identiques entre APIs : AFS · CP4 · CFG · C3A · C12 · NV5 · NV4 · NV3 · NV2 · NV1 (du plus faible au plus élevé). Référentiel canonique servi par offres-emploi." },
  { name: "Organismes du réseau pour l'emploi", apis: ["orientation-usager","rendez-vous-partenaires","gestion-sanctions-rsa"], collision: false, shared: true, description: "Codes FT (France Travail) · CD (Conseil départemental) · CAF · ML (Mission locale) · CE (Cap emploi) · IND (Indéterminé). Variations mineures selon le cas d'usage : RDV-partenaires ajoute DCD (délégation CD) et omet CAF ; la décision d'orientation exclut IND." },
  { name: "Freins à l'emploi (8 dimensions)", apis: ["diagnostic-usager","orientation-usager"], collision: false, shared: true, description: "Les mêmes 8 dimensions partagées entre les deux APIs : santé · logement · mobilité · famille · finances · numérique · administratif-juridique · français-calcul. Orientation expose un résumé (impact fort / faible / aucun) ; Diagnostic expose le détail (situations constatées + objectifs). C'est la vue grosse maille vs la vue fine maille du même sujet." },
...
// Newly published APIs — flagged with a "new" badge in the UI
const NEW: string[] = [
  "experiences-declarees-employeur",
  "cadre-vie-communes",
  "competences-professionnelles",
...
function stripMarkdown(text: string): string {
  return text
    ...
    .replace(/`,`/g, "`, `")                    // code-span list: add space after comma
...
async function main() {
  ...
  apis.sort((a, b) => {
    ...
    const t = a.title.localeCompare(b.title, "fr");
    if (t !== 0) return t;
    // Same title (e.g. Orientation Usager v1 & v2): newest version first
